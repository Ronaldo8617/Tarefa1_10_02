...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    if (c >= 'A' && c <= 'Z') {
        ...
        // Caractere desconhecido (espaço ou outro)
        index = 0; // Usa o padrão de "nada"
    }
...
void ssd1306_draw_line(ssd1306_t *ssd, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, bool value) {
  int dx = abs(x1 - x0);
  int dy = abs(y1 - y0);
